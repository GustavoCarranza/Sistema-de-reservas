...
include('../../../Modelos/Base/base_datos.php');//Conexion a la base de datos 
    $salida = "";
    $buscar_reservaciones = "SELECT * FROM reservas_eventos E INNER JOIN eventos EV ON E.name_evento = EV.id_evento ORDER BY name_evento,horario_reserva,fecha_reserva" ; //Consulta para solicitar todos los datos de la tabla
    ...
    if(isset($_POST['consulta'])){
        ...
        $buscar_reservaciones = "SELECT * FROM reservas_eventos E INNER JOIN eventos EV ON E.name_evento = EV.id_evento WHERE CONCAT(nombre_evento, ' ' ,fecha_reserva) LIKE '%".$buscar_caracteres."%' OR huesped LIKE '%".$buscar_caracteres."%' OR apellidos LIKE '%".$buscar_caracteres."%' OR solicitante LIKE '%".$buscar_caracteres."%' ORDER BY fecha_reserva,horario_reserva";
    }
    ...
    if($resultado->num_rows > 0){
        ...
        while($arreglo = $resultado->fetch_assoc()){//creamos un arregalo y o asociamos con la variable que tiene alojada la consulta 
            $salida.="<tr>
                    ...
                    <a href='../../../Controladores/Eventos/Recepcion/eliminar_reservaciones.php?id_reserva=". $arreglo['id_reserva_E']."' class='btn_eliminar eliminar_reservacion'><i class='bx bx-trash'></i></a>
                    ...
                    include('../../../Vistas/Views/Views_modals/Recepcion/Modal_eventos.php');//Modal para editar informacion del practicante 
                }
            $salida.="</tbody></table>";
...
<script src="../../../Vistas/js/Recepcion/Editar_info_eventos.js"></script>