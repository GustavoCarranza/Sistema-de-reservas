...
include('../../../../Modelos/Base/base_datos.php');//Conexion a la base de datos 
    $salida = "";
    $buscar_reservaciones = "SELECT * FROM reservas_eventos E INNER JOIN eventos EV ON E.name_evento = EV.id_evento WHERE fecha_reserva < CURDATE() ORDER BY name_evento,horario_reserva" ; //Consulta para solicitar todos los datos de la tabla
    ...
    if($resultado->num_rows > 0){
        ...
        while($arreglo = $resultado->fetch_assoc()){//creamos un arregalo y o asociamos con la variable que tiene alojada la consulta 
            $salida.="<tr>
                    ...
                    <a href='../../../../Controladores/Eventos/Reservaciones/reservas_pasadas/eliminar_reservacion.php?id_reserva=". $arreglo['id_reserva_E']."' class='btn_eliminar eliminar_reservacion'><i class='bx bx-trash'></i></a>
                    
                    "./*Boton para cambiar descanso*/"
                    <button type='submit' class='btn_editar' data-bs-toggle='modal' data-bs-target='#editar_reserva". $arreglo['id_reserva_E']."'><i class='bx bx-edit'></i></button> 
